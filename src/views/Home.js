...
export default function Home({ navigation }) {
  ...
  const handleSearch = () => {
    navigation.navigate("Buscar", {search: searchText});
  };
  ...
  return (
    ...
    >
      <Text style={{ ...styles.label, textAlign: "center" }}>
        Pesquisar por livros, artigos e outros materiais acadêmicossss
      </Text>
      ...
      <View>
        ...
        <ScrollView horizontal={true} showsHorizontalScrollIndicator={true}>
          <View style={styles.itemContainer}>
            {data.map((item, idx) => (
              <TouchableWithoutFeedback onPress={() => handleClick(item)} key={idx}>
              <View style={styles.item}>
                <Image source={item.imagem} style={styles.tinyLogo} />
                <Text style={styles.bookTitle}>{item.titulo}</Text>
                <Text style={styles.bookInfo}>{item.autor}</Text>
                <Text style={styles.bookInfo}>{item.categoria}</Text>
                <Text style={styles.bookInfo}>Ano de publicação: {item.ano}</Text>
              </View>
      ...
      <View>
        ...
        <ScrollView horizontal={true} showsHorizontalScrollIndicator={true}>
          <View style={styles.itemContainer}>
            {data.sort((a, b) => a.titulo < b.titulo ? 1 : a.titulo > b.titulo ? -1 : 0).map((item, idx) => (
              <TouchableWithoutFeedback onPress={() => handleClick(item)} key={idx}>
              <View style={styles.item}>
                <Image source={item.imagem} style={styles.tinyLogo} />
                <Text style={styles.bookTitle}>{item.titulo}</Text>
                <Text style={styles.bookInfo}>{item.autor}</Text>
                <Text style={styles.bookInfo}>Categoria: {item.categoria}</Text>
                <Text style={styles.bookInfo}>Ano de publicação: {item.ano}</Text>
              </View>
              </TouchableWithoutFeedback>
            ))}
      ...
      <View>
        ...
        <ScrollView horizontal={true} showsHorizontalScrollIndicator={true}>
          <View style={styles.itemContainer}>
            {data.sort((a, b) => a.autor < b.autor ? 1 : a.autor > b.autor ? -1 : 0).map((item, idx) => (
              <TouchableWithoutFeedback onPress={() => handleClick(item)} key={idx}>
              <View style={styles.item} key={idx} onPress={() => handleClick(item)}>
                <Image source={item.imagem} style={styles.tinyLogo} />
                <Text style={styles.bookTitle}>{item.titulo}</Text>
                <Text style={styles.bookInfo}>{item.autor}</Text>
                <Text style={styles.bookInfo}>{item.categoria}</Text>
